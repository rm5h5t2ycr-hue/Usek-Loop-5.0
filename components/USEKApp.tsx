...
export default function USEKApp() {
  const [activeTab, setActiveTab] = useState<string>('carpool');
  ...
  const [newItem, setNewItem] = useState<{ item: string; course: string }>({ item: '', course: '' });
  ...
  const handleCarpoolSubmit = () => {
    if (!carpoolData.type || !carpoolData.location || !carpoolData.time) return;
  ...
  const handleBookRoom = () => {
    if (!bookingData.date || !bookingData.startTime || !bookingData.endTime || !bookingData.people || !bookingData.studentId) {
