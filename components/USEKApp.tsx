...
export default function USEKApp() {
  ...
  const [chatMessages, setChatMessages] = useState<
    { id: number; user: string; message: string; time: string }[]
  >([
    {
      id: 1,
      user: 'Sarah M. - Engineering Senior',
      message: 'Does anyone know when registration for next semester opens?',
      time: '2 hours ago'
    },
    {
      id: 2,
      user: 'John K. - Business Junior',
      message:
        'It usually opens in mid-November. Check your email for exact dates!',
      time: '1 hour ago'
    }
  ]);
  const [newMessage, setNewMessage] = useState('');
  const [items, setItems] = useState<
    { id: number; user: string; item: string; course: string; available: boolean }[]
  >([
    {
      id: 1,
      user: 'Mike T.',
      item: 'Engineering Mathematics Notes',
      course: 'MATH 201',
      available: true
    },
    {
  ...
  const [matches, setMatches] = useState<
    { name: string; type: string; location: string; time: string; people: number }[]
  >([]);
  ...
  const handleCarpoolSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      alert(
        `✅ Green USEK notified:\n${match.people} ${
          match.people > 1 ? 'people' : 'person'
        } arriving at ${carpoolData.time}`
      );
    }, 1000);
  };
  ...
  const handleBookRoom = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    alert(
      `✅ Study room booked successfully!\n\nDate: ${bookingData.date}\nTime: ${bookingData.startTime} - ${bookingData.endTime}\nPeople: ${bookingData.people}\nStudent ID: ${bookingData.studentId}\n\nYou'll receive a confirmation email shortly.`
    );
    setBookingData({
      date: '',
      startTime: '',
              ...
              <form onSubmit={handleCarpoolSubmit} className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() =>
                        setCarpoolData({ ...carpoolData, type: 'driver' })
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setCarpoolData({ ...carpoolData, type: 'passenger' })
                      }
                      className={`p-4 border-2 rounded-lg transition-all ${
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCarpoolData({ ...carpoolData, time: e.target.value })
                    }
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    required
                  />
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        <Hash className="inline" size={14} /> {item.course}
                      </p>
